...
\begin{itemize}[nosep]
\item \textbf{主要贡献：} 主导医院原始数据的清洗、处理与多模态配对，构建包含1,834例样本的图文数据集，完成患者级去重与独立划分；主导实验设计、代码实现、全部实验及论文撰写。
\item \textbf{研究内容与成果：} 基于Qwen3-VL-8B-Instruct构建多模态大模型框架AdaPUV，通过LoRA对视觉编码器、视觉语言连接模块与语言模型进行参数高效微调，融合三类眼底影像与文本。系统开展14种模态配置评估、有／无文本条件下的跨模态零样本迁移（3×3迁移矩阵）、模态与文本消融及5\%--100\%训练数据规模实验，分析文本引导、缺失模态鲁棒性与数据效率。完整图文输入较无文本配置的准确率从88.30\%提升至96.28\%，AUROC达0.9893；仅用20\%--30\%训练数据仍达93.09\%准确率，外部验证准确率达94.52\%。
\end{itemize}
\projectsep
